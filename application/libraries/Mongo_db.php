...
class Mongo_db
{
	...
	protected $_configData = array(
		...
		'persist' => true, 'persist_key' => 'mongoqb',
		'replica_set' => false, 'query_safety' => 'w');
	...
	public function __construct($connect = true)
	{
		...
		$defaultConfig = $ci->config->item('default');
		$auth = $defaultConfig['mongo_auth'];
		$user = $defaultConfig['mongo_username'];
	...
	/**
	 *  Mongo switch object id to string
	 *  @param array array of objects
	 *  @return array of objects with string id
	 */
	public function switchId($arrays){
		foreach ($arrays as &$map)
		{
			$map['id'] = $map['_id']->{'$id'};
			unset($map['_id']);
		}
	 ...
	 *
	 * @aero - customized to allow array or wheres
	 *
			...
			foreach ($wheres as $where => $value)
			{
				if(isset($wheres[0])){
					foreach($value as $key => $sub_val){
						$this->wheres['$or'][] = array($key => $sub_val);
					}
				}else{
	 ...
	 * @return \MongoQB\Builder
	 */
	public function orWhereLike($wheres = array())
	{
		if (count($wheres) > 0)
		{
			if (!isset($this->wheres['$or']) or !is_array($this->wheres['$or']))
			...
			foreach ($wheres as $where => $value)
			{
				...
				$this->wheres['$or'][] = array($where => new \MongoRegex($regex));
			}
		}
	 ...
	 * @return Mongo_db
	 */
	public function whereExists($field = '', $exists = TRUE)
	{
		$this->_whereInit($field);
		$this->wheres[$field]['$exists'] = $exists;
		return $this;
	}
	...
	public function get($collection = '', $returnCursor = false)
	{
		...
		$cursor = $this->_dbhandle->{$collection}->find($this->wheres, $this->_selects)->limit($this->_limit)->skip($this->_offset)->sort($this->_sorts);
		...
		return $this->switchId($documents);
	}
	...
	public function count($collection = '')
	{
		...
		$count = $this->_dbhandle->{$collection}->find($this->wheres)->limit($this->_limit)->skip($this->_offset)->count();
	...
	public function insert($collection = '', $insert = array(), $options = array())
	{
		...
		$options = array_merge(array($this->_querySafety => true), $options);
	...
	public function delete($collection = '')
	{
		...
		try
		{
			$this->_dbhandle->{$collection}->remove($this->wheres, array(
					$this->_querySafety => true, 'justOne' => true));
	...
	public function deleteAll($collection = '')
	{
		...
		{
			$this->_dbhandle->{$collection}->remove($this->wheres, array(
					$this->_querySafety => true, 'justOne' => false));
	...
	public function command($query = array())
	{
		try
		{
			$execute = $this->_connection->admin->command($query);
	...
	{
		$this->_queryLog = array('collection' => $collection,
				'action' => $action, 'wheres' => $this->wheres,
				'updates' => $this->updates, 'selects' => $this->_selects,
