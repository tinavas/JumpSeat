    ...
    (function($) {
        ...
        Selectorator = (function() {
            function Selectorator(element, options) {
                ...
                this.options = extend(extend({}, $q.selectorator.options), options);
                this.cachedResults = {};
            }

            Selectorator.prototype.query = function(selector) {
                var base;
                return (base = this.cachedResults)[selector] || (base[selector] = $q(selector.replace(/#([^\s]+)/g, "[id='$1']")));
            };
            ...
            Selectorator.prototype.generateAncestor = function() {
                ...
                for (i = 0, len = ref.length; i < len; i++) {
                    ...
                    for (j = 0, len1 = selectors.length; j < len1; j++) {
                        ...
                        parentSelectors = new Selectorator($q(parent), this.options).generateSimple(null, false);
                        for (k = 0, len2 = parentSelectors.length; k < len2; k++) {
                            parentSelector = parentSelectors[k];
                            $q.merge(results, this.generateSimple(parentSelector, true, isFirst));
                        }
                    }
                    isFirst = false;
                }
            ...
            Selectorator.prototype.getIdSelector = function(tagName) {
                ...
                if (typeof id === "string" && !(/\d/.test(l)) && !contains(id, this.getIgnore('id')) && id !== '') {
            ...
            Selectorator.prototype.getClassSelector = function(tagName) {
                ...
                return map(classes, function(klazz) {
                    if (klazz && !(/\d/.test(l)) && !contains(klazz, invalidClasses)) {
        ...
        $q.fn.selectorator = function(options) {
            return new Selectorator($q(this), options);
        };
        $q.fn.getSelector = function(options) {
